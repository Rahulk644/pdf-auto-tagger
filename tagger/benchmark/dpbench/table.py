"""TEDS table similarity (replicated from opendataloader-bench, MIT).

The TEDS computation derives from PubTabNet ("Image-based table recognition",
Zhong et al., arXiv:1911.10683; https://github.com/ibm-aur-nlp/PubTabNet).
TEDS scores structure + cell text via APTED tree-edit distance; TEDS-S scores
structure only. `<th>` is normalized to `<td>` and `thead`/`tbody` are stripped,
so header role does NOT affect the score — TEDS-S is pure grid topology.
"""
...
class TEDSEvaluator(object):
    """Tree-Edit-Distance-based Similarity (PubTabNet)."""
...
def _convert_headers_to_cells(node) -> None:
    for header in node.xpath(".//th"):
        header.tag = "td"


def calc_table_score(gt_string: str, pred_string: str, evaluator: TEDSEvaluator) -> float:
    ...
    return evaluator.evaluate(refined_pred, refined_gold)
...
def evaluate_table(gt: str, pred: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    gt_tables = extract_tables(gt_with_html or "")
    pred_tables = extract_tables(pred_with_html or "")
    ...
    teds_s_score = calc_table_score(gt_data, pred_data, TEDSEvaluator(structure_only=True))
    teds_score = calc_table_score(gt_data, pred_data, TEDSEvaluator(structure_only=False))
    return teds_score, teds_s_score


__all__ = ["evaluate_table"]
