"""Convert Markdown tables into HTML snippets (shared by all dp-bench metrics).

Faithful replication of opendataloader-bench's src/converter_markdown_table.py
(MIT) for score parity. A pipe-table (header + `---` separator + rows) becomes an
HTML `<table>`; already-HTML tables and all other lines pass through unchanged.
"""
...
def convert_to_markdown_with_html_tables(markdown: Optional[str]) -> Optional[str]:
    """Convert Markdown pipe-tables in ``markdown`` into HTML tables."""
    if not markdown:
        return markdown
